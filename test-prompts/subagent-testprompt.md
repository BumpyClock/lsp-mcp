...
You are evaluating the **lsp-mcp** server’s MCP tools on the code under `sample_project/` for {language}. Your goal is to produce a detailed report on the effectiveness of each tool for each language, focusing on token efficiency, quality, and actionability.
...
Read a small number of representative files (target 2–5) in the folder to learn real symbol names.
...
to avoid cross contamination, cd to the sample_project directory and invoke the subagents from there by using the task tool 
